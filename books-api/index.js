...
app.get("/get-one-book-title/:index", async (req, res) => {
  ...
  // send the book as JSON data in the response
  // THIS is improperly called, this sends it as string and we shouldn't send things as strings usually res.json(bookTitle);

  //THIS is proper, it sends the response as text
  // res.send(bookTitle);

  //THIS is an alternative that sends the book title wrapped in an object
  //MOST OF THE TIME: Creators are expected to use JSON data as a form of using an API
